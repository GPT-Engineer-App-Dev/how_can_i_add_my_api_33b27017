  ...
  const handleSearch = async () => {
    ...
    const searchResults = await Promise.all(
      translatedQueries.map(async (translatedQuery, index) => {
        const response = await fetch(`https://www.googleapis.com/customsearch/v1?key=${import.meta.env.VITE_GOOGLE_API_KEY}&cx=${import.meta.env.VITE_GOOGLE_SEARCH_ENGINE_ID}&q=${encodeURIComponent(translatedQuery)}`);
        const data = await response.json();
        return {
